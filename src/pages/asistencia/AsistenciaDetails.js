...
export default function AsistenciaDetails({ darkMode }) {
    ...
    const [asistanceData, setAsistenceData] = useState(null);
    ...
    const fieldsToDisplay = [
        { label: "Fecha de trabajo", key: "workDate" },
        { label: "Hora de entrada", key: "timeEntry", format: (value) => (value===null ? "No Disponible" : value) },
        { label: "Hora de salida", key: "departureTime", format: (value) => (value===null ? "No Disponible" : value) },
        { label: "A tiempo", key: "onTime", format: (value) =>  value ? "Si" : "No" },
        { label: "Retardo", key: "retard", format: (value) => value ? "Si" : "No" },
        { label: "Ausencia justificada", key: "justifiedAbsence", format: (value) => value ? "Si" : "No" },
        { label: "Ausencia injustificada", key: "unjustifiedAbsence", format: (value) => value ? "Si" : "No" },
                    ...
                    <Card shadow={false} className={`w-full max-w-screen ${cardBgColor}`}>
                        ...
                        >
                            ...
                            <div className="flex w-full flex-col gap-0.5">
                                ...
                                <Typography className="text-blue-gray dark:text-gray-300">{asistanceId}</Typography>
                            </div>
                        </CardHeader>
                        <CardBody className="ml-12 mb-10 mr-10 mt-4 p-0">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                                {renderFields(fieldsToDisplay, asistanceData)}
                            </div>
                        </CardBody>
                    </Card>
                </div>
