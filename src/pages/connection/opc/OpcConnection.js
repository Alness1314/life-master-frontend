...
export default function OpcConnection({ darkMode }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}${process.env.REACT_APP_API_PREFIX}/opc-conexion`, {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/measurement-systems/${id}`, {
          method: "DELETE",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(responseData.message || "Error al eliminar el sistema de medición");
        }
  ...
  return (
    <div className="p-0 m-0">
      ...
      <Typography variant="paragraph" className={`mb-2 ${subTextColor}`}>
        Administra las conexiones OPC Server
      </Typography>
